...
function test_convergence(ns = [4, 8, 16, 32, 64, 128, 256, ], visualize = false)
    println("--------------------------------------------------------------------------------")
...
# ns, results = test_convergence([7, 15, 21, 25, 30, 39, 45, 54, 67, 90, 111, 123, 135, 197, 213, 233, 257, 290, ])    #
# ns, results = test_convergence([4, 6, 9, 15, 22, 33, 46, 70])
# ns, results = test_convergence([Int(round(2.0^i)) for i in 2:7])
# ns, results = test_convergence([Int(round(1.75^i)) for i in 3:9])
# ns, results = test_convergence([Int(round(1.5^i)) for i in 4:9])
ns, deflections = test_convergence([Int(round(1.25^i)) for i in 9:15])
...
using FinEtools
using FinEtools.AlgoBaseModule: richextrapol
using Main.RichardsonExtrapolationUQ: richextrapol_uq, gci_uq
using CSV

@show results = richextrapol_uq(deflections ./ phun("ft"), L ./ ns ./ phun("ft"))

h = [r.elementsize for r in results]
x = [r.estim for r in results]
e = [r.estim_ad_x_2 for r in results]

@show [x e]

b = [r.beta for r in results]
eb = [r.beta_ad_x_2 for r in results]
...
Coordinates([0.0, 4.5], [-0.301482, -0.301482])
)
push!(plots, p)
@pgf ax = Axis(
{
...
ymax = -0.24,
grid = "major",
xlabel = "Element size [ft]",
ylabel = "Deflection [ft]",
}, 
plots...
);
...
ns, deflections = test_convergence([Int(round(2^i)) for i in 2:7])

@show results = gci_uq(deflections ./ phun("ft"), L ./ ns ./ phun("ft"))

h = [r.elementsize for r in results]
x = [r.w_1 for r in results]
e = [r.gci * r.w_1 for r in results]
